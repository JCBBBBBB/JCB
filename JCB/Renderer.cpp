...
void Renderer::Render()
{
    ...
    uint32 stride = sizeof(VertexColor); // 정점 하나의 크기
...
void Renderer::CreateGeometry()
{
    ...
    //vertex
    {
        ...
        _vertices[0].color = { 1.f,0.f,0.f,0.f };

        _vertices[1].position = { -0.5f, 0.5f, 0.f };
        _vertices[1].color = { 0.f,1.f,0.f,0.f };

        _vertices[2].position = { 0.5f, -0.5f, 0.f };
        _vertices[2].color = { 0.f,0.f,1.f,0.f };

        _vertices[3].position = { 0.5f, 0.5f, 0.f };
        _vertices[3].color = { 0.f,0.f,0.f,0.f };
    }
...
void Renderer::CreateVertexBuffer()
{
    ...
    ZeroMemory(&desc, sizeof(desc));
    desc.ByteWidth = static_cast<uint32>(sizeof(VertexColor) * _vertices.size());
...
void Renderer::CreateShaderResourceView()
{
    ...
    HRESULT hr = LoadFromWICFile(L"Skeleton.png", WIC_FLAGS_NONE, &data, image);
    assert(SUCCEEDED(hr));
